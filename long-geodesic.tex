...
We illustrate the power of the tools by proving the theorem below for all convex surfaces including smooth surfaces, polyhedra and surfaces less convenient for working with standard methods.
...
\subsection*{Angles in path metric spaces}
The intrinsic metric on convex surfaces has certain comparison property, which we about to discuss.
A short but comprehensive introduction to the subject was written by Alexander Alexandrov in \cite{alexandrov1941}.
...
A metric is called geodesic if any two points $p,q\in \Sigma$ can be joined by a curve with length $|p-q|_\Sigma$;
...
and denoted as 
$[x y z]$.
Given a triple $x,y,z\in \Sigma$ many different triangles with these vertices may exist, 
...
The angle $\mangle\hinge{x}{y}{z}$ of the hinge is defined as the limit of model angles for small triangle sling along the sides of hinge to its vertex. 
That is,
...
It turns out, for the metrics considered in this paper this limits exists, see \textbf{[Good reference]}.
...
the comparison and supplementary property, then we say that $\Sigma$
has \emph{non-negative curvature in the sense of Alexandrov}.%
...
In other words, if $\Sigma$ has non-negative curvature in the sense of Alexandrov, then geodesics in $\Sigma$ can not bifurcate.
...
It is easy to show that from the comparison property it follows
\[\pm f'(t)\le -\cos[\phi_\pm(t)]\]
at any $t$ where the the derivative $f'(t)$ is defined.

\begin{thm}{Exercise}
Prove that this inequality holds for any surfaces were angles are well defined.
\end{thm}

%
Applying the supplementary property we get that $\phi_-+\phi_+=\pi$, hence $\cos\phi_+ +\cos\phi_-=0$.
Therefore the two inequalities above imply two identities
\begin{equation}
	\label{eq:first variation}
\pm f'(t)=-\cos[\phi_\pm(t)]
	\tag{${*}$}
\end{equation}
...
is a sphere with non-negatively curved metric in the sense of Alexandrov.
\end{thm}

In fact, as it was prove by Alexandrov in \cite{alexandrov1950},
the converse of this theorem also holds
\textbf{Wrong ref!}
if one consider convex plane figure as a convex body.
...
it was proved earlier by Alexandrov also in \cite{alexandrov1950}.
The proof is qute simple, it goes essentially the same way as the Cauchy's rigidity theorem.

\begin{thm}{Rigidity theorem}\label{Rigidity theorem}
Convex polyhedrons in $\EE^3$ with isometric surfaces are congruent. 
\end{thm}

\subsection*{Curvature and area}
...
If think about $\Sigma$ as about the boundary of convex body in $\mathbb{R}^3$ then the curvature measure of an set $U \in \Sigma$ is the Lebesgues measure of the image of $U$ under the Gaussian map.
...
We also will use the following area comparison, it follows easily from the comparison property.
