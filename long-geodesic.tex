...
Let $d$ be any metric on the sphere $\mathbb{S}^2$.
Then any $\varepsilon>0$ there is a $\delta>0$, that any simple curve in $(\mathbb{S}^2,d)$ of length smaller than $\delta$ bounds a region of diameter at most $\varepsilon$.
\end{thm}
...
By compactness of $\mathbb{S}^2$
we can pass to a subequence of $\gamma_n$  which converges to a point, say $p$. 

Since the diameters of $A_n$ and $B_n$ are at least $\eps$, we can choose the points $a_n\in A_n$ and $b_n\in B_n$ on the distance at least $\tfrac\eps{10}$ from $p$.
Pass to a subsequence of $\gamma_n$ so that $a_n$ and $b_n$ converge,
denote by $a$ and $b$ their limits.

Note that for all large $n$ the point $a$ can be connected to $a_n$ in $A_n$ 
and analogously the point $b$ can be connected to $b_n$ in $B_n$.
It follows that any curve from $a$ to $b$ must intesect all $\gamma_n$ for all large $n$.
Since $\gamma_n$ converges to $p$, any curve from $a$ to $b$ passes thru $p$.
That is, $a$ and $b$ lie in the different connected components of $\mathbb{S}^2\backslash\{p\}$, a contradiction.
\qeds
