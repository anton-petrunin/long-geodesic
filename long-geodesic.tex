...
each $\gamma_n$ cuts sphere into regions $A_n$ and $B_n$ of diameter at least $\varepsilon$. 
By compactness of $\mathbb{S}^2$
we can pass to a subequence of $\gamma_n$  which converges to a point, say $p$. 

For all large $n$, we can choose points $a_n\in  A_n$ and $b_n\in B_n$
on the distance at least $\tfrac\eps{10}$ from $p$.
Pass to a subsequence of $\gamma_n$ so that $a_n$ and $b_n$ converge,
denote by $a$ and $b$ their limits.

Note that any curve from $a$ to $b$ must intesect all $\gamma_n$ for all large $n$.
Since $\gamma_n$ converges to $p$, it follows that any curve from $a$ to $b$ passes thru $p$.
