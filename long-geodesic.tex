...
Since the surface  of a convex body in $\mathbb{R}^3$ is homeomorphic to sphere,
the following lemma implies that these conditions are equivalent.

\begin{thm}{Lemma}\label{Lemma:diameter-perimeter}
...
\parit{Proof.}
Suppose opposite, fix a sequence of curves $\gamma_n$ withs $\length\gamma_n\to 0$ as $n\to\infty$ such that 
each $\gamma_n$ cuts sphere into open regions $A_n$ and $B_n$ of diameter at least $\varepsilon$. 
By compactness of $\Sigma$ there is a subsequence of $\gamma_n$ which converges to a point, say $p$. 
We can pass to the subsequence and not that we can assume that all of $\gamma_n$ pass though $p$, by adding a short arcs to each of the curve.

If length of $\gamma_n$ less than $\varepsilon/3$, each region contains a disk with centers $a_n$ and $b_n$ and radius $\varepsilon/3$ (if not, than any two points in the region can be connected by a path of length $\varepsilon$ consisting from paths from them to $\gamma_n$ and a arc of $\gamma_n$).

Pass to a subsequence of $\gamma_n$ so that $a_n$ and $b_n$ converge, denote by $a$ and $b$ their limits.
Note that for all sufficiently big $n$ the domains $A_n$ and $B_n$ should contain the disks of radius $\varepsilon/4$ with centers at $a$ and $b$ correspondingly.
Since $a$ and $b$ are differ from $p$, there is a path $\gamma_{ab}$ from $a$ to $b$ avoiding $p$, on the other hand this path should intersects all $\gamma_{n}$ for sufficiently big $n$, because $a$ and $b$ lie in different regions formed by $\gamma_{n}$.
But it is not possible when length of the curves $\gamma_{n}$ became less than distance from $p$ to the path~$\gamma_{ab}$.	
\qeds
