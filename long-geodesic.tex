...
The intrinsic metric on convex surfaces has certain comparison property, which is called \emph{nonnegative curvature in the sense of Alexandrov}.
The idea is to present a generalization of Euclidean plane geometry,
where instead of equality in the cosine rule we write the following inequality

{

\begin{wrapfigure}[2]{r}{21 mm}
...
The angle $\mangle\hinge{x}{y}{z}$ of the hinge is defined as the limit of model angles for triangles sliding along the sides of hinge to its vertex. 
That is,
...
We say that \emph{suplementary property} holds for two supplementary hinges $\hinge{p}{x}{z}$ and $\mangle\hinge{p}{y}{z}$ if the angles $\hinge{p}{x}{z}$ and $\mangle\hinge{p}{y}{z}$ are defined and
...
Assume that any point of the surface $\Sigma$ admits a neighborhood $U$ such that the angles of all hinges in $U$ are defined and satisfy 
...
It was proved by Alexandrov himself \cite{alexandrov:devel} 
...
Further, assume $\Sigma$ has non-negative curvature in the sense of Alexandrov,
$\gamma$ is a geodesic in $\Sigma$, 
and $p$ is arbitrary point in $\Sigma$.
...
at any $t$ such that the derivative $f'(t)$ is defined (see  \cite[XI \S 2 (7)]{ aleksandrov1948vnutrennnyaya}). 


By the supplementary property $\phi_-\z+\phi_+=\pi$;
hence $\cos\phi_+ +\cos\phi_-=0$.
Therefore the two inequalities above imply so called \emph{first variation formula}; that is,
\begin{equation}
...
\end{thm}
Another proof of this theorem by dissection on small triangles can be found in \cite[X \S 1]{ aleksandrov1948vnutrennnyaya}.
...
In fact, as it was prove by Alexandrov in \cite[III \S 3]{aleksandrov1948vnutrennnyaya},
...
This hard theorem was proved by Pogorelov in \cite{pogorelov}.
We will use the following weaker statement proved by Alexandrov earlier 
\cite[VI \S 5]{aleksandrov1948vnutrennnyaya};
its proof goes essentially the same way as the Cauchy's rigidity theorem.
...
By the globalization theorem (\ref{Globalization theorem})
...
\bibitem{LS-globalization} Lang, U.; Schroeder, V. \emph{On Toponogov's comparison theorem for Alexandrov spaces.} \texttt{arXiv:1207.3668 [math.MG]}.
