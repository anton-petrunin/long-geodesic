...
Our theorem was announced by Jin-Ichi Itoh and Costin V\^{\i}lcu \cite[remark above Cor. 2.2]{itoh-vilcu}, but the proof was not written.
...
As a motivation, we recommend an other classical book of Alexandrov \cite{aleksandrov2005} --- it is fun to read, it worth reading and once red, it should be easy to read \cite{aleksandrov1948vnutrennnyaya} as well as most of papers on the subject.
Some related topics discussed in popular books; see for example \cite[Lectures 24 and 25]{tabachnikov-fuks}.
...
The surface of any convex body in $\EE^3$,
...
it will look like a surface if you can walk on both sides of the figure but can not pass from one side directly to the other side.
%it will look like a surface if you can walk on both sides of the figure but can not go pass thru it.
...
The functional $\kappa$ uniquely extends to non-negative measure, the so called \index{curvature measure}\emph{curvature measure}, defined on all Borel subsets in the interior of $\Sigma$.
...
Assume that the surface $\Sigma$ of a convex body $K$ in $\EE^3$
admits an arbitrarily long simple closed geodesic.
...
\parit{Proof.}
We will come to contradiction by showing that there is a point which cuts the surface of $\Sigma$ into connected non-empty parts.
...
Note that $D$ is locally convex in $\Sigma$; that is, sufficiently short minimizing geodesic with the ends in $D$ lies completely in $D$.
...
%\bibitem{proofs-from-the-book} M. Aigner, G. M. Ziegler, \emph{Proofs from The Book.} Springer-Verlag, Berlin, 2014.
...
\bibitem{aleksandrov2005} A.~D.~Alexandrov, \emph{Convex polyhedra.} Springer-Verlag, Berlin, 2005.
...
\bibitem{tabachnikov-fuks} D. Fuchs, S. Tabachnikov,
...
J. Itoh and
C. V\^{\i}lcu, 
\newblock On the length of simple closed quasigeodesics on convex surfaces. 
\newblock \emph{C. R. Math. Acad. Sci. Paris} 343 (2006), no. 4, 259--264. 
